...
UART_HandleTypeDef huart4;							/*!< UART Handler for SIMCOM7600*/
...
//		}
		...
		while(!UartTransmitReady);
		if(HAL_UART_Transmit_IT(&huart4, buffer, tmpLen)!= HAL_OK){
			Error_Handler();
		}
...
//			while(!UartTransmitReady);
		...
		while(!UartTransmitReady);
		if(HAL_UART_Transmit_IT(&huart4, (uint8_t*)buffer, buffer_size)!= HAL_OK){
			Error_Handler();
		}
		UartTransmitReady = RESET;
	}
...
void Start_Receive_IT(void){
	if(HAL_UART_Receive_IT(&huart4, &temp, 1)!= HAL_OK){
		Error_Handler();
	}
}
...
void Stop_Receive_IT(void){
	if(HAL_UART_AbortReceive_IT(&huart4)!= HAL_OK){
		Error_Handler();
	}
}
/**
