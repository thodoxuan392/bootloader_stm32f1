...
void MQTT_Start(){
	...
	//Clear_Reiceive_Buffer();
	sprintf(mqtt_client.clientid,"\"%s\"",CONFIG_get()->device_id);
...
void MQTT_Wait_For_Accquire_Client(){
	switch (Get_AT_Result()) {
		...
		case AT_ERROR:
			Clear_AT_Result();
			LOG("MQTT Config failed: ACCQUIRE_CLIENT");
			mqtt_state = MQTT_MAX_STATE;
			break;
		default:
			Clear_AT_Result();
...
void MQTT_Wait_For_Connect(){
	if(is_Mqtt_TimeOutFlag()){
		switch (Get_AT_Result()) {
			...
			case AT_ERROR:
				Clear_AT_Result();
				LOG("MQTT Config failed: CONNECT");
				mqtt_state = MQTT_MAX_STATE;
				SCH_Delete_Task(task_MQTT_Timeout_ID);
				break;
			default:
...
void MQTT_Wait_For_Input_Topic(){
	if(is_Mqtt_TimeOutFlag()){
		...
		switch (Get_AT_Result()) {
			...
			case AT_ERROR:
				Clear_AT_Result();
				LOG("MQTT Config failed: INPUT_TOPIC");
				mqtt_state = MQTT_MAX_STATE;
				SCH_Delete_Task(task_MQTT_Timeout_ID);
				break;
			case AT_SUBCRIBE_MESSAGE:
...
void MQTT_Wait_For_Payload(){
	if(is_Mqtt_TimeOutFlag()){
		...
		switch (Get_AT_Result()) {
			...
			case AT_ERROR:
				Clear_AT_Result();
				LOG("MQTT Config failed: PAYLOAD");
				mqtt_state = MQTT_MAX_STATE;
				SCH_Delete_Task(task_MQTT_Timeout_ID);
				break;
			case AT_SUBCRIBE_MESSAGE:
	...
	if(is_Mqtt_TimeOutFlag()){
		...
		switch (Get_AT_Result()) {
			case AT_OK:
				...
				break;
			case AT_ERROR:
				LOG("MQTT Config failed: IN_PAYLOAD");
				mqtt_state = MQTT_MAX_STATE;
				SCH_Delete_Task(task_MQTT_Timeout_ID);
				Clear_AT_Result();
				break;
...
void MQTT_Wait_For_Publish(){
	if(is_Mqtt_TimeOutFlag()){
		...
		switch (Get_AT_Result()){
			...
			case AT_ERROR:
				Clear_AT_Result();
				LOG("MQTT Config failed: PUBLISH");
				mqtt_state = MQTT_MAX_STATE;
				SCH_Delete_Task(task_MQTT_Timeout_ID);
				break;
			case AT_SUBCRIBE_MESSAGE:
